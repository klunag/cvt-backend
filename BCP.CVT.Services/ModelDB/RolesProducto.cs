//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace BCP.CVT.Services.ModelDB
{
    using System;
    using System.Collections.Generic;
    
    public partial class RolesProducto
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public RolesProducto()
        {
            this.RolesProductoAmbiente = new HashSet<RolesProductoAmbiente>();
        }
    
        public int RolesProductoId { get; set; }
        public Nullable<int> ProductoId { get; set; }
        public string Rol { get; set; }
        public string GrupoRed { get; set; }
        public Nullable<bool> FlagActivo { get; set; }
        public Nullable<bool> FlagEliminado { get; set; }
        public string Descripcion { get; set; }
        public string RolSeguridad { get; set; }
        public Nullable<int> EstadoRol { get; set; }
        public Nullable<System.DateTime> FechaCreacion { get; set; }
        public string CreadoPor { get; set; }
        public Nullable<System.DateTime> FechaModificacion { get; set; }
        public string ModificadoPor { get; set; }
        public Nullable<System.DateTime> FechaEliminacion { get; set; }
        public string EliminadoPor { get; set; }
        public Nullable<int> TipoCuenta { get; set; }
    
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<RolesProductoAmbiente> RolesProductoAmbiente { get; set; }
    }
}
