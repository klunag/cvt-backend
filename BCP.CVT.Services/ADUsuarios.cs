...
namespace BCP.CVT.Services
{
    public class ADUsuarios
    {
        ...
        public List<GraphUserMetadata> GetUserbyMatricula(string matricula)
        {
            var search_text = string.Format("?$select=displayName,givenName,id,mail,onPremisesSamAccountName,userPrincipalName&$filter=onPremisesSamAccountName eq '{0}'&$count=true", matricula);
            var URL_API = string.Format(Settings.Get<string>("Graph.API.Users"), search_text);
            log.DebugFormat("Url búsqueda: {0}", URL_API);
