...
namespace BCP.CVT.Services.Service
{
    public class ProductoSvc : ProductoDAO
    {
            ...
            try
            {
                using (var ctx = GestionCMDB_ProdEntities.ConnectToSqlServer())
                {
                    ...
                    else
                    {
                        ...
                        string descripcionProducto = ObtenerCambiosProducto(objeto, entidad, objDomData, objSDoData, objTCiData, lstArqData);
        ...
        public override List<ProductoDTO> GetProducto(string nombre, int? dominioId, int? subDominioId, bool Activo, int pageNumber, int pageSize, string sortName, string sortOrder, out int totalRows, bool withCantidadTecnologias = false)
        {
            try
            {
                totalRows = 0;
                ...
                using (var scope = new TransactionScope(TransactionScopeOption.RequiresNew, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted }))
                {
                    using (var ctx = GestionCMDB_ProdEntities.ConnectToSqlServer())
                    {
                        var registros = (from u in ctx.Producto
                                         ...
                                         || u.Descripcion.ToUpper().StartsWith(filtro.ToUpper())
                                         || u.Fabricante.ToUpper().StartsWith(filtro.ToUpper())
                                         || string.Concat((u.Fabricante ?? ""), " ", (u.Nombre ?? "")).StartsWith(filtro.ToUpper())
                                         || string.IsNullOrEmpty(filtro)) && (u.FlagActivo)
                                         select new CustomAutocomplete()
                                         {
                                             Id = u.ProductoId.ToString(),
                                             // Fabricante = u.Fabricante,
                                             //Nombre = u.Nombre,
                                             Descripcion = string.Concat((u.Fabricante ?? "") + " " + (u.Nombre ?? "")),
                                             value = string.Concat((u.Fabricante ?? "") + " " + (u.Nombre ?? "")),

                                         }).ToList(); //.OrderBy(x => x.Fabricante).ThenBy(x => x.Nombre).ToList();
        ...
        public override List<ProductoDTO> GetListaAutoCompletarProducto(string descripcion, string dominioIds = null, string subDominioIds = null)
        {
            try
            {
                ...
                {
                    cnx.Open();
                    using (var comando = new SqlCommand("[CVT].[USP_Buscar_Producto_Autocomplete_GU]", cnx))
                    {
                        comando.CommandTimeout = 0;
                        comando.CommandType = System.Data.CommandType.StoredProcedure;
                        comando.Parameters.Add(new SqlParameter("@pfiltro", descripcion));
            ...
            catch (Exception ex)
            {
                ...
                throw new CVTException(CVTExceptionIds.ErrorTecnologiaDTO
                    , "Error en el metodo: GetListaAutoCompletarProducto()"
                    , new object[] { null });
            }
        }
        ...
        string ObtenerCambiosProducto(ProductoDTO objChange, Producto objData = null, Dominio objDomData = null, Subdominio objSDoData = null, TipoCicloVida objTCiData = null, List<Arquetipo> lstArqData = null)
        {
